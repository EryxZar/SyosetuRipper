...
// @version      1.3
// @description  Botones de descarga por capítulo y botón flotante "Descargar todos" (multi página). Genera un EPUB por capítulo (incluye IMÁGENES y afterword) y los empaqueta en un ZIP, con barra de progreso de texto. Incluye fallback y logs para casos como n8829lf.
// @author       EryxZar
...
(function () {
  ...
  console.log('[SyosetuRipper] Script cargado.');
  ...
  // Extrae el número de capítulo del enlace: /n3709ho/123/ → "123"
  ...
  // ====== CONSTRUCTOR ZIP (para EPUB interno y ZIP externo) ======
...
<package xmlns="http://www.idpf.org/2007/opf" version="2.0" unique-identifier="BookId">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    ...
    <dc:creator>${escapeHtml(meta.author || 'Desconocido')}</dc:creator>
...
${manifestItems}
  ...
  // ====== DESCARGA Y EXTRACCIÓN DE CAPÍTULO ======
  async function fetchAndExtractChapter(url) {
    ...
    const title = titleElem ? titleElem.innerText.trim() : 'Capítulo';

    // Incluimos TODOS los bloques de texto, incluyendo el afterword
    ...
    for (const block of blocks) {
      ...
      // Procesar imágenes de este bloque
      ...
      for (const img of imgs) {
        ...
        try {
          ...
          if (!imgRes.ok) {
            console.warn('Error al descargar imagen', absUrl, imgRes.status);
            img.remove();
            continue;
          }
          ...
          console.warn('Fallo al obtener imagen', absUrl, e);
          img.remove();
        }
      }
  ...
  // ====== OBTENER TODOS LOS CAPÍTULOS (TODAS LAS PÁGINAS DEL ÍNDICE) ======
  async function collectAllChapterLinks() {
    const result = [];
    const seen = new Set(); // para evitar repetir capítulos (pages clonadas)
    ...
    while (page <= maxPages) {
      ...
      console.log('[SyosetuRipper] Cargando índice página', page, ':', url);
      const res = await fetch(url, { credentials: 'include' });
      if (!res.ok) {
        console.warn('[SyosetuRipper] Error HTTP en índice página', page, res.status);
        break;
      }
      ...
      console.log('[SyosetuRipper] Página', page, '- enlaces encontrados:', links.length);

      if (!links.length) {
        console.log('[SyosetuRipper] No hay lista de capítulos en esta página, se detiene el recorrido.');
        break;
      }
      ...
      links.forEach(a => {
        ...
        if (!/\/\d+\/?$/.test(hrefAttr)) return; // solo capítulos /ncode/numero/
      ...
      // Si esta página no aportó capítulos nuevos, asumimos que es un índice repetido
      if (newCount === 0) {
        console.log('[SyosetuRipper] Página', page, 'no añadió capítulos nuevos. Se asume índice repetido y se detiene.');
        break;
      }
    ...
    console.log('[SyosetuRipper] Total capítulos recolectados (fetch):', result.length);
    return result;
  }

  // ====== FALLBACK: capítulos solo desde la página actual ======
  function collectChaptersFromCurrentPage() {
    ...
    console.log('[SyosetuRipper] Fallback DOM - capítulos encontrados en esta página:', result.length);
    return result;
  }
  ...
  console.log('[SyosetuRipper] Título de la novela:', novelTitle);

  // ====== ÍNDICE: BOTONES "descargar" AL LADO DE CADA CAPÍTULO ======
  const chapterLinks = document.querySelectorAll('.p-eplist__sublist a.p-eplist__subtitle');
  console.log('[SyosetuRipper] Capítulos visibles en DOM:', chapterLinks.length);
  if (chapterLinks.length) {
    chapterLinks.forEach(a => {
      ...
      btn.addEventListener('click', async (e) => {
        ...
        try {
          showProgress('Descargando capítulo (texto + imágenes + afterword)...');
          ...
          btn.textContent = '⚠️';
          showProgress('Error al descargar este capítulo. Mira la consola (F12 → Console).');
          hideProgress(4000);
        } finally {
          btn.disabled = false;
        }
  ...
  function createFloatingButtons() {
    ...
    // Si estamos en una página de capítulo (/nXXXXxx/1/), botón para ese solo capítulo
    if (isChapterPage) {
      const singleBtn = document.createElement('button');
      singleBtn.className = 'tm-float-btn';
      singleBtn.textContent = '📥 Descargar este capítulo';
      singleBtn.addEventListener('click', async () => {
        if (singleBtn.disabled) return;
        singleBtn.disabled = true;
        singleBtn.textContent = '📥 Descargando...';
        ...
        try {
          showProgress('Descargando capítulo actual (texto + imágenes + afterword)...');
          const { title, bodyXhtml, resources } = await fetchAndExtractChapter(chapterUrl);
          const epubBytes = buildEPUBBytes({
            title,
            author: novelTitle,
          ...
          const filename = sanitizeFilename(baseName) + '.epub';
          downloadBlob(blob, filename);
          showProgress('EPUB listo. Descarga iniciada.');
          hideProgress(4000);
        } catch (err) {
          console.error('Error descargando capítulo actual:', err);
          showProgress('Error al descargar este capítulo. Mira la consola (F12 → Console).');
          hideProgress(5000);
        } finally {
    ...
    // Botón para descargar TODOS los capítulos
    ...
    allBtn.addEventListener('click', async () => {
      ...
      allBtn.textContent = '📚 Trabajando...';

      try {
        showProgress('Buscando todos los capítulos en todas las páginas del índice...');
        let allChapters = [];
        try {
          allChapters = await collectAllChapterLinks();
        } catch (e) {
          console.warn('[SyosetuRipper] Error en collectAllChapterLinks, usando fallback DOM:', e);
        }

        if (!allChapters.length) {
          // Fallback: solo la página actual
          showProgress('No se pudieron obtener capítulos por fetch.\nUsando sólo los capítulos visibles en esta página...');
        ...
        if (!total) {
          showProgress('No se encontraron capítulos.');
          hideProgress(4000);
        ...
        for (let i = 0; i < total; i++) {
          ...
          showProgress(
            `Descargando capítulos: ${i + 1} / ${total}\n${chap.href}`
          );
          ...
          const baseName = (chapNum ? chapNum + '- ' : '') +
                           (title || chap.text || `capitulo_${i + 1}`);
          const name = sanitizeFilename(baseName) + '.epub';

          files.push({ name, data: epubBytes });
        }

        showProgress('Empaquetando ZIP con todos los EPUB...');
        const zipBytes = buildZipUint8(files);
        const zipBlob = new Blob([zipBytes], { type: 'application/zip' });
        const zipName = sanitizeFilename(novelTitle || 'novela') + '_todos_epub.zip';
        downloadBlob(zipBlob, zipName);
        showProgress('ZIP listo. Descarga iniciada.');
        hideProgress(4000);
      } catch (err) {
        console.error('Error en descarga masiva:', err);
        showProgress('Error en la descarga masiva. Mira la consola (F12 → Console).');
        hideProgress(5000);
      } finally {
        allBtn.disabled = false;
        allBtn.textContent = '📚 Descargar todos';
